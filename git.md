...
首先强调我们公司用的是企业级的gitlab

每一个项目单独创建线上仓库，前后端分离的项目分别建两个仓库，以后缀作为区分(front或者back)

每个项目仓库主要有5中分支：master、hotfix、release、develop、feature。

master作为主分支也是线上生产环境的版本严禁commit，并且只有小组leader有merge的权限

deleop分支作为开发分支，可以由开发人员进行merge
...
完成release分支会完成以下三个操作：1、合并release分支到master；2、给master打上版本的标签；3、release回归到develop分支。

当发现master上有bug时，开一个hotfix，完成后合并到master分支。

当有紧急的bug需要修复是，没有时间开hotfix分支，则直接回退版本号，以修复bug的优先级为最高
